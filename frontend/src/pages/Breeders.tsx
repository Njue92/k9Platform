...
import { Heart, Calendar, FileText, DollarSign, Dog as DogIcon, Loader2, Phone, GraduationCap } from "lucide-react";
...
const Breeders = () => {
    ...
    const fetchData = async () => {
      try {
        if (user) {
          ...
          // Unauthenticated visitor: fetch all active dogs added by breeders (public endpoint)
          const dogsRes = await dogsAPI.listPublicByRole('breeder');
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <section className="bg-gradient-to-br from-secondary-light via-background to-primary-light py-16">
        <div className="container mx-auto px-4">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-secondary/10 rounded-full mb-4">
              ...
              <span className="text-sm font-medium text-secondary">Breeding Module</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              Dog Breeder Inventory
            </h1>
            <p className="text-lg text-muted-foreground mb-6">
              Comprehensive breeding program management with complete tracking of dogs,
              litters, health records, and financial performance.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <section className="py-8 border-b border-border">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Total Dogs", value: stats.total_dogs ?? 0, icon: Heart },
                { label: "Active Dogs", value: stats.active_dogs ?? 0, icon: Calendar },
                { label: "Total Litters", value: stats.total_litters ?? 0, icon: FileText },
                { label: "Monthly Income", value: `Ksh.${(stats.monthly_income ?? 0).toLocaleString()}`, icon: DollarSign },
              ].map((stat, index) => (
                <motion.div
                  key={index}
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">
                {user
                  ? "Add dogs from your Breeder Dashboard to see them here."
                  : "No breeder dogs are currently listed. Check back soon."}
              </p>
              {user && (
                    ...
                    <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
                      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                        ...
                        <div className="absolute top-3 right-3">
                          <Badge className="bg-secondary text-white shadow-lg capitalize">
                            {dog.status}
                          </Badge>
                        </div>
                      </div>
